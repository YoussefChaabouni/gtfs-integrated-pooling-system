...
def environmental_request_handling(riders_list,system):
    
    riders_data_list = []
    drivers_data = []
    requests = read_data("requests.pkl")
    for i in range(number_of_drivers):
        drivers_data.append({"id" : i, "boarding_alighting_list":[],"riders":[]})
    ...
    for i in tqdm(range(len(riders_list))):
        ...
        if itinerary != -1 and system != "no carpooling":
            drivers_data = single_driver_data(response,itinerary,drivers_data,id)
            #print(drivers_data[:50])
    save_data(riders_data_list,'riders_simulation_data')

    return riders_data_list,drivers_data

def single_driver_data(response,itinerary,drivers_data,rider_id):
   # for i in range(number_of_drivers):
    ...
    for item in response['plan']['itineraries'][itinerary]['legs']:
        if "route of carpooler number" in str(item["route"]):
            ...
            copy_of_drivers_data[id-1]["riders"].append(rider_id)
